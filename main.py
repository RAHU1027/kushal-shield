...
async def run_client(data):
    ...
    async def handler(event):
        ...
        # Security: Delete Bad Content
        if any(word in text for word in BAD_KEYWORDS):
            try: await event.delete()
            except: pass
            return
        
        # 3 Reactions for each account
        try:
